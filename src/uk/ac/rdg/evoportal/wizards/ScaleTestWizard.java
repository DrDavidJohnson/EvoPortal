...
import uk.ac.rdg.evoportal.pages.ScaleTests;
import uk.ac.rdg.evoportal.tasks.ScaleTestSubmitTask;
import uk.ac.rdg.util.NexusUtil;
/**
 *
 * @author david
 */
public class ScaleTestWizard extends Wizard {
    
    private BPAnalysisSettings analysis = new BPAnalysisSettings();
    private BPScaleTestSettings test = new BPScaleTestSettings();
    transient Logger LOG = Logger.getLogger(ScaleTestWizard.class.getName());
...
//        }
    ...
    private final class TestStep extends WizardStep {

        IModel<BPAnalysisSettings> analysisModel = new Model<BPAnalysisSettings>(analysis);

        public TestStep() {
            super();
            setTitleModel(new ResourceModel("teststep.title"));
            setSummaryModel(new StringResourceModel("teststep.summary", this, analysisModel));
            add(new TextField("test.name", String.class).setRequired(true));
            DropDownChoice repsDropDown = new DropDownChoice("test.repetitions", getIntSeqList(1, 10));
            add(repsDropDown);
            DropDownChoice itDropDown = new DropDownChoice("analysis.iterations", Arrays.asList(new Integer[]{
                100, 250, 500, 750, 1000, 2000, 3000, 4000, 5000, 10000}));
            add(itDropDown);
            DropDownChoice scaleNodesMinDropDown = new DropDownChoice("test.scaleNodesMin", getIntSeqList(1, 100));
            add(scaleNodesMinDropDown);
            DropDownChoice scaleNodesIncDropDown = new DropDownChoice("test.scaleNodesIncrements", getIntSeqList(1, 100));
            add(scaleNodesIncDropDown);
            DropDownChoice scaleNodesMaxDropDown = new DropDownChoice("test.scaleNodesMax", getIntSeqList(1, 100));
            add(scaleNodesMaxDropDown);
            DropDownChoice ppnDropDown = new DropDownChoice("test.ppn", Arrays.asList(new Integer[]{2, 4}));
            add(ppnDropDown);
        }
    }
    ...
    private class ConfirmationStep extends WizardStep implements ICondition {
        ...
        public ConfirmationStep() {
            ...
            add(new TextArea("analysis.controlBlock"));
            add(new Label("test.name"));
            add(new Label("test.repetitions"));
            add(new Label("test.scaleNodesMin"));
            add(new Label("test.scaleNodesIncrements"));
            add(new Label("test.scaleNodesMax"));
            add(new Label("test.ppn"));
        }
    ...
    public void onFinish() {
        super.onFinish();
        String user = ((LoginSession)getSession()).getUsername() ;
        File nexusFile = new File(GlobalConstants.getProperty("local.fileroot") + "/" + user + "/mynexusfiles/", analysis.getFileName());
        try {
            ...
            ScaleTestSubmitTask scaleTestSubmitTask = new ScaleTestSubmitTask(
                    tempFile,
                    user,
                    test.getName(),
                    analysis.getIterations(),
                    test.getScaleNodesMin(),
                    test.getScaleNodesMax(),
                    test.getScaleNodesIncrements(),
                    test.getRepetitions(),
                    analysis.getFileName(),
                    analysis.getControlBlock());
            scaleTestSubmitTask.run();
            if (!tempFile.delete()) {
